...
export function ActivityCard({
  ...
  /** Statement style: rows under "Today" / "Yesterday" / a date, like the reference's home. */
  grouped?: boolean;
}) {
    ...
    <section className="rounded-[20px] border bg-card px-4 pb-1 pt-1">
      ...
      {rows.length === 0 ? (
        ...
        <div>
          {groups.map((g) => (
            <div key={g.label ?? 'all'}>
              {g.label && <p className="pt-3 text-xs font-medium text-muted-foreground">{g.label}</p>}
