    ...
    <nav aria-label="Quick actions" className="grid grid-cols-4 gap-3">
      {ACTIONS.map(({ href, label, icon: Icon }) => (
        <Link
          key={href}
          href={href}
          className="flex min-h-[72px] flex-col items-center justify-center gap-2 rounded-[20px] border bg-card text-xs font-medium transition-[background-color,color,border-color,transform] duration-200 hover:bg-accent/40 active:scale-[0.97] active:border-primary active:bg-primary active:text-primary-foreground"
        >
