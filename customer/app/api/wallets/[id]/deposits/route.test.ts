...
function body(b: unknown) {
  return new Request('http://localhost/api/wallets/w1/deposits', {
    method: 'POST',
...
describe('POST /api/wallets/[id]/deposits', () => {
  it('forwards amount/note to NestJS and returns the row', async () => {
    ...
    const res = await POST(body({ amount: 5000, note: 'top up' }), ctx('w1'));
