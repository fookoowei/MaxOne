...
function req(path: string, opts?: { session?: boolean; access?: boolean; refresh?: boolean }) {
  const parts: string[] = [];
  if (opts?.session) {
    const value = encodeURIComponent(JSON.stringify({ id: 'u1', email: 'a@b.c', role: 'user' }));
  ...
  return new NextRequest(new URL(`http://localhost:3300${path}`), { headers });
}
...
describe('proxy auth-presence guard', () => {
  it('redirects an unauthenticated user from a protected page to /login', async () => {
    ...
    expect(res.headers.get('location')).toBe('http://localhost:3300/login');
  });
