...
function build(userRow: any) {
  const users = {
    findByIdRaw: jest
      .fn()
      .mockResolvedValue(userRow && { id: 'u1', ...userRow }),
...
describe('TwoFactorService.setup', () => {
  it('refuses when 2FA is already enabled (no silent downgrade)', async () => {
    ...
    await expect(svc.setup('u1', 'a@b.c')).rejects.toBeInstanceOf(
      ConflictException,
    );
...
describe('TwoFactorService.verifyAndEnable', () => {
  it('rejects a wrong code', async () => {
    ...
    await expect(svc.verifyAndEnable('u1', '000000')).rejects.toBeInstanceOf(
...
describe('TwoFactorService.verifyForLogin', () => {
  const enabled = {
    totpSecret: 'SECRET',
    totpEnabled: true,
    totpRecoveryHashes: [sha('abcdef1234')],
  };
  ...
  it('accepts an unused recovery code and consumes it', async () => {
    ...
    expect(users.consumeRecoveryHash).toHaveBeenCalledWith(
      'u1',
      sha('abcdef1234'),
    );
  });
