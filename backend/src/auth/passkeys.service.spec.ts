...
jest.mock('@simplewebauthn/server', () => ({
  generateRegistrationOptions: jest
    .fn()
    .mockResolvedValue({ challenge: 'REG-CHAL', rp: { id: 'localhost' } }),
  verifyRegistrationResponse: jest.fn(),
  generateAuthenticationOptions: jest
    .fn()
    .mockResolvedValue({ challenge: 'AUTH-CHAL' }),
...
function build(passkey: any = {}, tokens: any = {}) {
  ...
  const tok = {
    issueWebAuthnChallenge: jest.fn().mockResolvedValue('chal.jwt'),
    verifyWebAuthnChallenge: jest.fn(),
    ...tokens,
  };
  return {
    svc: new PasskeysService(prisma as any, tok, config as any),
    prisma,
    tok,
  };
}
...
describe('PasskeysService registration', () => {
  it('returns options for our RP + a challenge token bound to the user, excluding existing passkeys', async () => {
    const { svc, tok } = build({
      findMany: jest
        .fn()
        .mockResolvedValue([{ credentialId: 'old', transports: ['internal'] }]),
    });
    ...
    const args = (swa.generateRegistrationOptions as jest.Mock).mock
      .calls[0][0];
    expect(args.rpID).toBe('localhost');
    expect(args.excludeCredentials).toEqual([
      { id: 'old', transports: ['internal'] },
    ]);
  });
  ...
  it('saves the public key + counter from registrationInfo.credential', async () => {
    ...
    const { svc } = build(
      { create },
      {
        verifyWebAuthnChallenge: jest
          .fn()
          .mockResolvedValue({ challenge: 'REG-CHAL', userId: 'u1' }),
      },
    );
    await svc.verifyRegistration(
      'u1',
      { id: 'cred1' } as any,
      'chal.jwt',
      'MacBook',
    ...
    expect(
      (swa.verifyRegistrationResponse as jest.Mock).mock.calls[0][0],
    ).toMatchObject({
  ...
  it('rejects a challenge that was issued to a different user', async () => {
    const { svc, prisma } = build(
      {},
      {
        verifyWebAuthnChallenge: jest
          .fn()
          .mockResolvedValue({ challenge: 'REG-CHAL', userId: 'u2' }),
      },
    );
    await expect(
      svc.verifyRegistration('u1', {} as any, 'chal.jwt'),
...
describe('PasskeysService authentication', () => {
  ...
  it('verifies, bumps the counter, and returns the owning userId', async () => {
    ...
    const { svc } = build(
      { findUnique: jest.fn().mockResolvedValue(row), update },
      {
        verifyWebAuthnChallenge: jest
          .fn()
          .mockResolvedValue({ challenge: 'AUTH-CHAL' }),
    ...
    expect(
      await svc.verifyAuthentication({ id: 'cred1' } as any, 'chal.jwt'),
    ).toBe('u1');
    expect(update).toHaveBeenCalledWith({
      where: { id: 'pk1' },
      data: { counter: 4, lastUsedAt: expect.any(Date) },
    });
    expect(
      (swa.verifyAuthenticationResponse as jest.Mock).mock.calls[0][0]
        .credential,
    ).toMatchObject({ id: 'cred1', counter: 3 });
  });
  ...
  it('rejects an unknown credential', async () => {
    ...
    await expect(
      svc.verifyAuthentication({ id: 'nope' } as any, 'chal.jwt'),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
  ...
  it('step-up: rejects a passkey that belongs to another user', async () => {
    ...
    await expect(
      svc.verifyAuthentication({ id: 'cred1' } as any, 'chal.jwt', 'u2'),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
});
...
describe('PasskeysService.remove', () => {
  it('deletes only your own passkey; 404 otherwise', async () => {
    ...
    await expect(svc.remove('u1', 'pk9')).rejects.toBeInstanceOf(
      NotFoundException,
    );
    expect(prisma.passkey.deleteMany).toHaveBeenCalledWith({
      where: { id: 'pk9', userId: 'u1' },
    });
  });
});
